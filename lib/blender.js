...
const {always, isNil, map, times} = require('ramda')

const lunr = require('lunr')
const pug = require('pug')
const Vinyl = require('vinyl')

const {waterfall: asyncWaterfall, each: asyncEach, map: asyncMap} = require('./async')
...
module.exports = function _Blender(state) {
	return stream => asyncWaterfall([
		always(Object.entries(state.files)),
		_middlewares(state),
		_htmlize(state),
		_index(state),
		asyncEach(([, file]) => stream.push(file)),
	])
}
